...
import serviceAccountKey from "./blog-website-a4302-firebase-adminsdk-hkey7-bc1ddf0dd9.json" assert { type: "json" }
...
const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        const date = new Date();
        const imageName = `${nanoid()}-${date.getTime()}${path.extname(file.originalname)}`;
        cb(null, imageName);
    }
});
...
// Image upload route
server.post('/get-upload-url', upload.single('image'), (req, res) => {
    if (!req.file) {
